...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):    
        ...
        # get GT image and bayer pattern mask
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, l1_loss(image, gt_image).mean(), loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background))
...
def prepare_output_and_logger(lp, opt, pp):    
    ...
    if TENSORBOARD_FOUND:
        tb_writer = SummaryWriter(lp.model_path)
    else:
